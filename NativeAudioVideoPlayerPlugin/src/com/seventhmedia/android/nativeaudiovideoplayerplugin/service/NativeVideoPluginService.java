...
public class NativeVideoPluginService extends Service implements
	...
	private int NOTIFICATION = R.string.NativeVideoPluginService_started;
	
	/**
     * Class used for the client Binder.
     * Because we know this service always runs in the same process as its clients, we don't need to deal with IPC.
     */
    public class NativeVideoPluginServiceBinder extends Binder {
    	public NativeVideoPluginService getService() {
            //return this instance of NativeVideoPluginService so clients can call public methods declared in this Service
            return NativeVideoPluginService.this;
        }
    }
    ...
    public static boolean checkIfServiceIsRunning( Context context ) {
	    ...
	    for ( RunningServiceInfo service : manager.getRunningServices( Integer.MAX_VALUE ) ) {
	        if ( NativeVideoPluginService.class.getName().equals( service.service.getClassName() ) ) {
	        	return true;
	        }
	    }
	    return false;
    ...
    public void startVideo( String dataSource, Boolean isMediaPlayerAutoPlay ) {
    	if ( !_isMediaPlayerStarted ) {
    		...
    		if ( android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.ICE_CREAM_SANDWICH ) {
    			...
    			_vitamioMediaPlayer = new io.vov.vitamio.MediaPlayer( NativeVideoPluginService.this );
    ...
    public void startVideo() {
    	startVideo( _dataSource, _isMediaPlayerAutoPlay );
    }
    ...
    public void playVideo() {
    	if ( ( _isMediaPlayerStarted ) && ( _isStreamReady ) ) {
    		Notification notification = getNotification( getText( R.string.NativeVideoPluginService_label_play ).toString() );
    ...
    public void pauseVideo() {
    	if ( ( _isMediaPlayerStarted ) && ( _isStreamReady ) ) {
    		Notification notification = getNotification( getText( R.string.NativeVideoPluginService_label_pause ).toString() );
    		if ( notification != null ) {
    			_notificationManager.notify( NOTIFICATION, notification );
    		}
    ...
    public boolean onUnbind( Intent intent ) {
        ...
        Toast.makeText( this, R.string.NativeVideoPluginService_stopped, Toast.LENGTH_SHORT ).show();
	...
	public IBinder onBind( Intent intent ) {
    	...
    	if ( ( intent != null ) && ( intent.getExtras() != null ) ) {
        	...
        	_dataSource = bundle.getString( NativeVideoPluginActivity.class.getName() + NativeVideoPluginActivity.EXTRA_VIDEO_PATH );
        	_isMediaPlayerAutoPlay = bundle.getBoolean( ( NativeVideoPluginActivity.class.getName() + NativeVideoPluginActivity.EXTRA_MEDIA_PLAYER_AUTO_PLAY ), false );
        	_activityNameToLaunch = bundle.getString( NativeVideoPluginActivity.class.getName() + NativeVideoPluginActivity.EXTRA_ACTIVITY_NAME_TO_LAUNCH );
        	_resultReceiver = bundle.getParcelable( NativeVideoPluginActivity.class.getName() + NativeVideoPluginActivity.EXTRA_RESULT_RECEIVER );
        }
    	
    	//tell the user we stopped the service
        Toast.makeText( this, R.string.NativeVideoPluginService_started, Toast.LENGTH_SHORT ).show();
    	
        //Display a notification about us starting.  We put an icon in the status bar.
        Notification notification = getNotification( getText( R.string.NativeVideoPluginService_label_loading ).toString() );
    ...
    private final IBinder _binder = new NativeVideoPluginServiceBinder();
    ...
    private Notification getNotification( String label ) {
    	String title = label;
    	String message = getText( R.string.NativeVideoPluginService_message ).toString();
    	String ticker = getText( R.string.NativeVideoPluginService_started ).toString();
    	...
    	if ( cls != null ) {
			...
			builder.setContentIntent( contentIntent )
						.setSmallIcon( R.drawable.ic_service_livestreamvideoplayer_clapper_small )			//required
						.setContentTitle( title )															//required
						.setContentText( message )															//required
						
						.setLargeIcon( BitmapFactory.decodeResource( res, R.drawable.ic_service_livestreamvideoplayer_clapper_large ) )
						.setTicker( ticker )
    ...
    private void initMediaPlayer() {
    	...
    	Notification notification = getNotification( getText( R.string.NativeVideoPluginService_label_ready ).toString() );
    	if ( notification != null ) {
    		_notificationManager.notify( NOTIFICATION, notification );
    	}
    ...
    private void closeMediaPlayerDueToError() {
    	if ( _resultReceiver != null ) {
    		_resultReceiver.send( NativeVideoPluginActivity.RESULT_CODE_SERVICE_ERROR, null );
    	}
    }
}